...
export default class GameScene extends Phaser.Scene {
	...
	create() { 
		...
		socket.on('currentPlayers', players => Object.keys(players).map(id => 
			players[id].playerId === socket.id ? this.addPlayer(this, players[id]) : this.addOtherPlayer(this, players[id])));
            
		socket.on('newPlayer', playerInfo => {
			this.addOtherPlayer(this, playerInfo);
		});

		socket.on('playerDisconnect', playerId => this.otherPlayers.getChildren().map(otherPlayer =>
			playerId === otherPlayer.playerId ? otherPlayer.destroy() : null));
